...
function initType0600(){
	$.ajax({
        url: 'pastMainGraph.action',
        type: 'POST',
        data: $("#MyForm").serialize(),
        dataType: 'json',
        success: function(data){
        	refresh60000(data);
        	pastLoadResultCompare(data);
        	pastLoadDateSearch(data);
        	$("#baseDate").val(data.type0600BaseDate);
        	$("#type0600SelectDateSearch").val($("#baseDate").val());
...
function pastLoadDateSearch(data){
    var type0600DateSearch= new Date(data.type0600Min);
    var opt1={
    		dateFormat: "yy.mm.dd (D)", 
    		defaultDate: type0600DateSearch, 
    		altField: "#type0600SelectDateSearch", 
    		altFormat: "yymmdd",
    		onChangeMonthYear: function(year, month, inst){
    			$("#type0600DateSearchWeather").val(year+""+timeformat(month)+"00");
    			type0600DateSearchWeather();
    			$("#type0600DateSearchWeather").val($("#type0600SelectDateSearch").val());
    		},
    		beforeShow: function(){
    ...
    $( "#datepicker3" ).val(dateformat(data.type0600Min));
    $( "#datepicker3" ).datepicker(opt1, $.datepicker.regional[ "ko" ]);

    $( "#type0600DateSearch" ).val(type0600DateSearch.getFullYear()+timeformat(type0600DateSearch.getMonth()+1)+timeformat(type0600DateSearch.getDate()));
    
};
// -------------------------------------------------------------------------

function pastLoadResultCompare(data){
...
function type0600DateWeather1(){
	$.ajax({
        url: 'pastDatePickerWeather.action',
        type: 'POST',
...
//-------------------------------------------------------------------------
function type0600DateSearchWeather(){;
	$.ajax({
	    url: 'pastDatePickerWeather.action',
	    type: 'POST',
	    data: $("#hiddenForm").serialize(),
	    ...
	    success: function(data){
	    	$(".ui-state-default").each(function(){
	    		for(var i=0; i<data.type01100[2].length; i++){
	    			if($(this).text()== (Number(data.type01100[2][i].baseDate.substring(6,8))+"")){
	    				$(this).attr('dataValue',$(this).text());
	    				if(data.type01100[2][i].holidayFalg == 'Y'){
	    					$(this).parent().attr('class','ui-datepicker-week-sun');
	    				}
	    				if(data.type01100[2][i].maxTemperature != null)
	    					$(this).append("<div><p id=" + "datepickerWeather"+ ">"+"("+data.type01100[2][i].maxTemperature+","+data.type01100[2][i].minTemperature+")</p></div>");
	    				break;
	    				}
	    			else{
	    				$(this).attr('dataValue',$(this).text());
...
function refresh60000(data){
    ...
    var fillflag = [true, true, true, true, true, true, true, true, false], 
    	...
    	$("#btnDateSearch").bind("click",function(){ 
        	$("#baseDate").val($("#type0600SelectDateSearch" ).val());
        	$("#type0600CompareDate1").val("");
        	$("#type0600CompareDate2").val("");
        	$.ajax({
                url: 'pastMainGraph.action',
                type: 'POST',
                data: $("#MyForm").serialize(),
